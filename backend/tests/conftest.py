...
@pytest.fixture
def test_app(app: FastAPI, database: Database, fake_llm: FakeLlmGateway) -> FastAPI:
    workflow = BugExtractionWorkflow(fake_llm)
    task_service = TaskService(database, bug_workflow=workflow)
...
@pytest.fixture
def slack_client(client: TestClient, test_app: FastAPI) -> TestClient:
    return client
